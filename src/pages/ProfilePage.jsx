...
export default function ProfilePage() {
  ...
  async function load() {
    ...
    if (own) {
      const [savedFaqs, followedFaqs, follows] = await Promise.all([api(`/users/${profileId}/saved-faqs`), api(`/users/${profileId}/followed-faqs`), api(`/users/${profileId}/follows`)]);
      ...
      <main className="profile-page">
        <section className="surface profile-header">
          ...
          <div>
            ...
            <b>{user.reputation} reputation</b>
          ...
          {own
            ...
            : <button className={`outline-button ${followedByViewer ? "active" : ""}`} onClick={toggleProfileFollow}>
                {followedByViewer ? <UserCheck size={15} /> : <UserPlus size={15} />}{followedByViewer ? "Following" : "Follow student"}
              </button>}
        </section>
        ...
        {editing && <form className="surface form-grid profile-edit" onSubmit={save}>
          <h2>Edit profile</h2>
          <Field label="Name" required><input value={form.name ?? ""} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field>
          <div className="form-row">
            <Field label="Branch (optional)"><select value={form.branch ?? ""} onChange={(event) => setForm({ ...form, branch: event.target.value })}><option value="">Select</option>{branches.map((branch) => <option key={branch}>{branch}</option>)}</select></Field>
            <Field label="Semester (optional)"><select value={form.semester ?? ""} onChange={(event) => setForm({ ...form, semester: event.target.value })}><option value="">Select</option>{[1, 2, 3, 4, 5, 6, 7, 8].map((semester) => <option key={semester}>{semester}</option>)}</select></Field>
          </div>
          <Field label={`Bio (optional - ${form.bio?.length ?? 0}/200)`}><textarea maxLength="200" value={form.bio ?? ""} onChange={(event) => setForm({ ...form, bio: event.target.value })} /></Field>
        ...
        {own && <ProfileList title="Saved FAQs" empty="No bookmarks yet.">
          {saved.map((faq) => <Link key={faq._id} to={`/faqs/${faq._id}`}>{faq.title}<small>{faq.category}</small></Link>)}
        </ProfileList>}
        {own && <ProfileList title="Followed FAQs" empty="Not following any FAQs yet.">
          {followed.map((faq) => <Link key={faq._id} to={`/faqs/${faq._id}`}>{faq.title}<small>{faq.category}</small></Link>)}
        </ProfileList>}
        {own && networkTab && <section className="surface network-panel">
